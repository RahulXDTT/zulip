...
class UnstarStreamMessagesTest(ZulipTestCase):
    def test_do_unstar_inaccessible_stream_messages(self) -> None:
        ...
        self.assertEqual(events[0]["event"]["operation"], "remove")
        self.assertEqual(events[0]["event"]["flag"], "starred")
        self.assertFalse(events[0]["event"]["all"])
        self.assertEqual(events[0]["users"], [hamlet.id])
        self.assertCountEqual(events[0]["event"]["messages"], [message_id1, message_id2])
    ...
    def test_do_unstar_inaccessible_stream_messages_multiple_streams(self) -> None:
        """
        Test that do_unstar_inaccessible_stream_messages removes the starred
        flag from messages across multiple inaccessible streams and sends a
        single event containing all the affected messages.
        """
        hamlet = self.example_user("hamlet")
        stream_name1 = "private_stream"
        stream_name2 = "private_stream_2"
        stream1 = self.make_stream(stream_name1, invite_only=True)
        stream2 = self.make_stream(stream_name2, invite_only=True)
        self.subscribe(hamlet, stream_name1)
        self.subscribe(hamlet, stream_name2)

        message_id1 = self.send_stream_message(hamlet, stream_name1, "test 1")
        message_id2 = self.send_stream_message(hamlet, stream_name2, "test 2")
        do_update_message_flags(hamlet, "add", "starred", [message_id1, message_id2])

        assert stream1.recipient_id is not None
        assert stream2.recipient_id is not None
        Subscription.objects.filter(
            user_profile=hamlet, recipient_id__in=[stream1.recipient_id, stream2.recipient_id]
        ).update(active=False)

        with self.capture_send_event_calls(expected_num_events=1) as events:
            count = do_unstar_inaccessible_stream_messages(
                hamlet, [stream1.recipient_id, stream2.recipient_id]
            )
